...
Divrg$x <- as.numeric(Divrg$divergence) / as.numeric(Divrg$day)
...
ggsave(file="figure_5_2.eps", plot=img, width=15, height=8)
...
morocco_omicron <- subset(morocco, grepl("Omicron", Nextstrain_clade, ignore.case = TRUE))
...
p <- pheatmap::pheatmap(sorted_matrix_ma,cluster_cols = FALSE, cluster_rows = TRUE)
p
...
save_pheatmap_pdf(p, "sorted_matrix_ma.eps", width=30, height=3)
```
...
library(data.table)
getwd()
world <- read.csv2('./results/world.tsv', sep='\t')
...
world_omicron <- world[world$Nextstrain_clade %in% c('21L','21K','22A','22B','22C','22D','22E','22F' ), ]
...
p <- pheatmap::pheatmap(var_heatmap,cluster_cols = FALSE, cluster_rows = TRUE, main="Heatmap", fontsize_row=9,fontsize_col=8, border_color="white", treeheight_row=5,  cellwidth = 9, cellheight = 15, )

save_pheatmap_pdf(p, "sorted_matrix_ma_wrl.eps", width=30, height=3)

```

# Boxplot
```{r}
morocco_omicron$Nextstrain_clade


# Calculate the first and third quartiles
morocco_omicron <- morocco_omicron[morocco_omicron$divergence <150,]

m <- ggplot(morocco_omicron, aes(x = Nextstrain_clade, as.numeric(divergence), fill = Nextstrain_clade))+
  geom_jitter(size=.3, shape=21, aes( color = Nextstrain_clade))+   
  geom_boxplot()+ labs(x='Omicron Variant', 'Divergence (Number of mutation)')
m
ggsave(file="boxplot_omicron ma.svg", plot=m, width=12, height=5)
...
omicrons$divergence <- as.numeric(omicrons$divergence)
...
omicron_21K <- ma_mt[ma_mt$Variant == 'MA Omicron 21K ',]$Mutation
omicron_21L <- ma_mt[ma_mt$Variant == 'MA Omicron 21L ',]$Mutation
omicron_22A <- ma_mt[ma_mt$Variant == 'MA Omicron 22A ',]$Mutation
omicron_22B <- ma_mt[ma_mt$Variant == 'MA Omicron 22B ',]$Mutation
omicron_22C <- ma_mt[ma_mt$Variant == 'MA Omicron 22C ',]$Mutation
omicron_22D <- ma_mt[ma_mt$Variant == 'MA Omicron 22D ',]$Mutation
omicron_22E <- ma_mt[ma_mt$Variant == 'MA Omicron 22E ',]$Mutation
omicron_22F <- ma_mt[ma_mt$Variant == 'MA Omicron 22F ',]$Mutation
...
library(VennDiagram)
vn <- venn.diagram(D, filename = NULL)
vn 
grid::grid.draw(vn)

ggsave(vn, file="ven.svg", device = "svg")
...
ma_omicron_21K <- mt[mt$Variant == 'MA Omicron 21K ',]$Mutation
ma_omicron_21L <- mt[mt$Variant == 'MA Omicron 21L ',]$Mutation
ma_omicron_22A <- mt[mt$Variant == 'MA Omicron 22A ',]$Mutation
ma_omicron_22B <- mt[mt$Variant == 'MA Omicron 22B ',]$Mutation
ma_omicron_22C <- mt[mt$Variant == 'MA Omicron 22C ',]$Mutation
ma_omicron_22D <- mt[mt$Variant == 'MA Omicron 22D ',]$Mutation
ma_omicron_22E <- mt[mt$Variant == 'MA Omicron 22E ',]$Mutation
ma_omicron_22F <- mt[mt$Variant == 'MA Omicron 22F ',]$Mutation

wr_omicron_21K <- mt[mt$Variant == 'WR Omicron 21K ',]$Mutation
wr_omicron_21L <- mt[mt$Variant == 'WR Omicron 21L ',]$Mutation
wr_omicron_22A <- mt[mt$Variant == 'WR Omicron 22A ',]$Mutation
wr_omicron_22B <- mt[mt$Variant == 'WR Omicron 22B ',]$Mutation
wr_omicron_22C <- mt[mt$Variant == 'WR Omicron 22C ',]$Mutation
wr_omicron_22D <- mt[mt$Variant == 'WR Omicron 22D ',]$Mutation
wr_omicron_22E <- mt[mt$Variant == 'WR Omicron 22E ',]$Mutation
wr_omicron_22F <- mt[mt$Variant == 'WR Omicron 22F ',]$Mutation
